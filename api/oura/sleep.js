...
function isNap(record) {
  ...
  const daytimeStart = startHour >= 8 && startHour < 20;
  return seconds < 3 * 3600 && daytimeStart;
}
...
function normalize(sleepRecords, dailySleepRecords) {
  ...
  const days = [...grouped.entries()].map(([day, dayPeriods]) => {
    ...
    return {
      ...
      allSleepSeconds: dayPeriods.reduce((sum, period) => sum + (period.totalSleepSeconds || 0), 0),
    };
  });

  for (const [day, score] of scoreByDay.entries()) {
    if (!grouped.has(day)) days.push({ day, sleepScore: score, mainSleep: null, naps: [], napCount: 0, napSleepSeconds: 0, allSleepSeconds: 0 });
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, error.status || 500, { ok: false, error: error.message || 'Unable to load Oura sleep data.' });
  }
}
